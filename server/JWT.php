...
class JWT
{
	...
	public function is_valid(string $jwt): bool
	{
		...
		// didnt want to deal with exp in a dev build, can be implemented in prod
	...
	public function decode($jwt): string
	{
		...
		$header = json_decode(base64_decode($token[0]), true); // Decode header
		$payload = json_decode(base64_decode($token[1]), true); // Decode payload
		$signature = $token[2];
		$base64_header = $this->encode(json_encode($header));
		$base64_payload = $this->encode(json_encode($payload));
